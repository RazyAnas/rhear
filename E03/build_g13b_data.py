...
OUT = os.path.expanduser("~/PS#2/handoff/data/g13b_20k")   # rebuilt with DNS5+US8K
...
# Verified against the only open-source model trained for this task:
#   Hush (huggingface.co/weya-ai/hush, Apache-2.0) -- "60% of training samples
#   include a competing human speaker at 12-24 dB SIR".
# G13 used 31% at an SIR that a build bug left at ~2 dB. Both are corrected.
ITF_PROB = 0.60
SIR_RANGE = (12.0, 24.0)
...
def main():
    ...
    n, rep = build(OUT, speech_roots, None, None, f"{C}/RIRS_NOISES",
                   ...
                   interferer_index=interferers(),
                   interferer_prob=ITF_PROB, sir_range=SIR_RANGE)
    print(f"\nwrote {n} -> {OUT}")
